  ...
  >
    {#each billboard as image, i}
      <div>
        ...
        <button class="btn" onclick={`my_modal_${image.type_of_match.slice(0, 3)}_${i+1}.showModal()`}>open modal</button>
        <dialog id={`my_modal_${image.type_of_match.slice(0, 3)}_${i+1}`} class="modal modal-bottom sm:modal-middle  bg-black/75">
          <div class="modal-box">
            <form method="dialog">
              <button class="btn btn-sm btn-circle btn-primary absolute right-2 top-2">✕</button>
            </form>
            <h3 class="text-2xl font-ibm italic text-primary text-pretty">{image.type_of_match}</h3>
