...
<div class="container flex w-full justify-between top-0 items-center absolute z-20 py-2 px-2 left-[50%] translate-x-[-50%]">
    <a href="/">
    <img src={urlForImage(logo).url()} class="w-28" alt="Logo 5LC">
    </a>
    <button on:click={() => (hidden2 = false)} class="btn btn-primary drawer-button md:hidden">Menu</button>
  ...
  <Drawer divClass='overflow-y-auto z-50 p-4 bg-primary' transitionType="fly" {transitionParams} bind:hidden={hidden2} id='sidebar2'>
  <div class='flex items-center'>
    ...
    <Sidebar>
      <SidebarWrapper divClass='overflow-y-auto py-4 px-0 rounded-sm w-full'>
        <SidebarGroup>
            {#each siteMap as {index, title}}
                <SidebarItem href={index} label={title} {spanClass} data-sveltekit-preload-data="hover"  on:click={() => (hidden2 = true)}>
                </SidebarItem>
            {/each}
