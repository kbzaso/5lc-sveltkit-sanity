...
>
{#if !$disclaimerEvent}
<p class="text-xs text-error text-center mt-2">Debes aceptar el código de conducta</p>
{/if}
<dialog
  id="my_modal_5"
...
>
  <div class="modal-box">
    ...
    <form action="?/pay" method="POST" use:enhance class="space-y-4">
      ...
      {#if canSubmitForm}
        <div
          class="text-sm text-gray-400 text-center flex h-fit w-full justify-center gap-4 items-center"
        >
          <span>Paga con tu banco</span>
          <span>
            <svg
              width="85"
              height="40"
